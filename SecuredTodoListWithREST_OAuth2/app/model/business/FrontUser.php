...
class FrontUser {
    ...
    static function enterMemberArea(){
        ...
        if($session->islogged()){
            if($session->getUser()->getAccountLevel() >= 0){
                ...
                throw new FrontUserException("Your account has been banned");
            }
        }
        throw new FrontUserException("You must be a member to see this page");
    }
    ...
    static function getRestUser($rest_token){
        try{
            ...
            $user = $userDB::getRestUser($rest_token);
    ...
    static function registerUser($user) {
        try {
            ...
            throw new $userException("Registation failed");
        }catch (Exception $ex) {
            $dbmanager = DIC::get(CONF_CLASS_DBMANAGER);
            $dbmanager::rollbackAuth();
            throw $ex;
    ...
    static function generateRestToken(){
        try{
            $dbmanager = DIC::get(CONF_CLASS_DBMANAGER);
            $dbmanager::startAuthTransaction();
            $session = DIC::get(CONF_CLASS_SESSION);
            $token = $session->generateRestToken();
            $userDB = DIC::get(CONF_CLASS_USERDB);
            $userDB::updateRestToken($session->getUser(), $token);
            $dbmanager::endAuthTransaction();
            return $token;
        } catch (Exception $ex) {
            $dbmanager = DIC::get(CONF_CLASS_DBMANAGER);
