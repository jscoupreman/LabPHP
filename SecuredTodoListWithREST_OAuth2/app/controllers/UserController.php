...
class UserController extends Controller {
    ...
    static function processLogin($redirect_url = null) {
        try {
            ...
            if (!$frontUser->isLogged()) {
                ...
                if(isset($redirect_url)){
                    header("Location: " . $router->getUrl($redirect_url));
                }else{
    ...
    static function showUserProfile() {
        try {
            $frontUser = DIC::get(CONF_CLASS_FRONTUSER);
            $frontUser::enterMemberArea();
            //$frontUser::generateRestToken();
            Controller::render('UserPages/profile');
        } catch (Exception $ex) {
            $log = DIC::get(CONF_CLASS_LOG);
            $log::logEvent($log::CRITICAL, $ex);
            Controller::render('ContentPages/void');
    ...
    static function generateRestToken(){
        try {
            $frontUser = DIC::get(CONF_CLASS_FRONTUSER);
            $frontUser::enterMemberArea();
            $token = $frontUser::generateRestToken();
            die($token);
        } catch (Exception $ex) {
            $log = DIC::get(CONF_CLASS_LOG);
            $log::logEvent($log::CRITICAL, $ex);
